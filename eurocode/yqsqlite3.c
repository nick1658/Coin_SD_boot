...
S16 test_read_r_code (S16 r_code)
{
	if (r_code == 0)
	{
		//my_println ("Read OP Complete");  
		return 0;
	}
	else if (r_code == 1)
	{
		my_println (" Parameter Error, Buf must be not null!!!");  
	}
	else if (r_code == 2)
	{
		my_println ("ECC Error, Read Op failed!!!");  
	}
	return -1;
}
S16 test_write_r_code (S16 r_code)
{
	...
	{
		my_println ("BAD BLOCK, Write failed!!!");  
	}
	else if (r_code == 3)
	{
		my_println ("BAD BLOCK, Mark failed!!!");  
	}
	else if (r_code == 4)
	{
		my_println ("BAD BLOCK, Mark Succeesfull!!!");  
	}
	return -1;
}
...
void initial_nandflash(void)    //nandflash
{
	...
	my_println ("sizeof (s_coin_parameter_value)   = %d", sizeof (s_coin_parameter_value));
	my_println ("sizeof (u_coin_cmp_value) = %d", sizeof (u_coin_cmp_value));
	
	my_println ("sizeof (para_set_value)   = %d", sizeof (para_set_value));
	my_println ("sizeof (u_coin_pre_value) = %d", sizeof (u_coin_pre_value));
	
	ASSERT(sizeof (para_set_value) > PAGE_BYTE_SIZE);
	ASSERT(sizeof (u_coin_pre_value) > PAGE_BYTE_SIZE);
	
	my_println ("Start Check Data ...!");
	...
	if(para_set_value.data.magic_num != MAGIC_NUM)    //不满足条件需要初始化    这里每个国家的信息都 初始化了 在界面更换硬币时不用担心值不确定
	{
		...
		para_set_value.data.rej_position = 2;
		...
		my_println ("erase block %d completed", PUBULIC_DATA_START_BLOCK_NUM);
		test_write_r_code (Nand_WritePage(PUBULIC_DATA_START_BLOCK_NUM, PUBULIC_DATA_START_PAGE_NUM, (U8 *)&para_set_value));
		my_println ("write block %d page %d completed", PUBULIC_DATA_START_BLOCK_NUM, PUBULIC_DATA_START_PAGE_NUM);
		...
		for (i = 0; i < COUNTRY_NUM; i++)
		{
			...
			my_println ("write block %d page %d completed", 
	...
	{
		my_println ("Check Data Completed!");
...
void ini_picaddr(void) //币种切换时的 初始化地址函数
{
	...
	my_env.coin_index = 0;
	dgus_tf1word(ADDR_CNTB,my_env.coin_index);	//initial addr on zixuexi jiemian coin name tubiao
	disp_preselflearn(pre_value.country[coinchoose].coin[my_env.coin_index].data.max0,pre_value.country[coinchoose].coin[my_env.coin_index].data.min0,
					...
					pre_value.country[coinchoose].coin[my_env.coin_index].data.max2,pre_value.country[coinchoose].coin[my_env.coin_index].data.min2);	//initial addr on zixuexi jiemian value 	
...
//			U8 str_db[20];
			...
			my_println("Block %d page = %d, db_total_item_num = %d",
			...
			db_item_info_temp->ID = para_set_value.data.rej_position;
		...
		{		
			...
			para_set_value.data.db_total_item_num = 0;
			write_para();
			my_println("delete block %d over", HISTORY_START_BLOCK_NUM);
			cmd ();
			break;
		}
		case DBDISPLAY:
		{
			...
			Uart0_Printf ("*******************************************************************************\n");
			Uart0_Printf ("Display pre page data, db_total_item_num = %d\n", para_set_value.data.db_total_item_num);
			Uart0_Printf("Block = %d page = %d addr = %d db_id = %d item_index = %d\n",
			...
			if(db_id >= 0){
				...
				if (r_code == 0){
					Uart0_Printf ("*******************************************************************************\n");
					Uart0_Printf ("|-------|----------------|----|--------------|----------------|-----------|\n");
					Uart0_Printf ("| Index |      Time      | ID | Total amount | Total Quantity |   Other   |\n");
					Uart0_Printf ("|-------|----------------|----|--------------|----------------|-----------|\n");
	...
	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////					
		...
		{	
			...
			Uart0_Printf ("*******************************************************************************\n");
			Uart0_Printf ("display next page data, db_total_item_num = %d\n", para_set_value.data.db_total_item_num);
			Uart0_Printf("Block = %d page = %d addr = %d db_id = %d item_index = %d\n",
				YQNDHISTORY_DB_ID_PAGE_ADDR / BLOCK_BYTE_SIZE, 
				(YQNDHISTORY_DB_ID_PAGE_ADDR - YQNDHISTORYBLOCK) / PAGE_BYTE_SIZE, 
				YQNDHISTORY_DB_ID_PAGE_ADDR, 
