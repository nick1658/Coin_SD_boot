...
//			Uart0_Printf("%d:%x ",uartcount,touchnum[uartcount]);
			uartcount++;
		}
...
//				{
//					Uart0_Printf("%02d ",uartcount);
//				}
//				Uart0_Printf("\n");
//				for( uartcount=0;uartcount<temp;uartcount++)//TSGET_NUM = 12
//				{
//					touchnum[uartcount]  = touchnum[uartcount];
//					Uart0_Printf("%02x ",touchnum[uartcount]);
//				}
//				Uart0_Printf("\n");
				/////////////////////////////////////////////////////////////////////////////////////
				uartcount = 0;
...
S16 test_repete(void)    //  判别 是不是重币的函数
{
	...
	ei = is_repeate (my_env.coin_index);
	
	if (ei == 0)
	{
		pre_value.country[coinchoose].coin[my_env.coin_index].data.max0 = coin_maxvalue0;
		pre_value.country[coinchoose].coin[my_env.coin_index].data.min0= coin_minvalue0;
		pre_value.country[coinchoose].coin[my_env.coin_index].data.max1 = coin_maxvalue1;
		pre_value.country[coinchoose].coin[my_env.coin_index].data.min1= coin_minvalue1;
		pre_value.country[coinchoose].coin[my_env.coin_index].data.max2 = coin_maxvalue2;
		pre_value.country[coinchoose].coin[my_env.coin_index].data.min2= coin_minvalue2;
		pre_value.country[coinchoose].coin[my_env.coin_index].data.std0 = std_ad0;
		pre_value.country[coinchoose].coin[my_env.coin_index].data.std1 = std_ad1;
		pre_value.country[coinchoose].coin[my_env.coin_index].data.std2 = std_ad2;
		//Write_value(my_env.coin_index); //写入  总函数  根据币种写 鉴别值 基准值
		write_para ();
		my_println ("Total learned coin number is %d", coinlearnnumber);
		my_println ("Save Complete");  
	...
	{
		my_println ("DA min0 = %d DA max0 = %d", coin_minvalue0, coin_maxvalue0);
		my_println ("DA min1 = %d DA max1 = %d", coin_minvalue1, coin_maxvalue1);
		my_println ("DA min2 = %d DA max2 = %d", coin_minvalue2, coin_maxvalue2);
		my_println ("ei = %d", ei);
	}		
	return ei;
...
void touchresult(void)      //根据接收到的  数 来决定 执行的任务
{
	...
	case ADDR_CRUN:  //地址ADDR_CRUN 0X06  按键返回值判断 
		if( (value == 0x01)){	 
			...
			my_env.workstep = 3;	
			Uart0_Printf("begin working\r\n");
		}else if( (value == 0x02)){
		  //A5 5A 06 83 00 06 01 00 02	  混计数界面 stop					
			runstep = 40;   //开始停机
			Uart0_Printf("end working %s %d\n", __FILE__, __LINE__);
		}
		break;
	...
	case ADDR_LRUN:  //地址ADDR_LRUN 0X3C  按键返回值判断 特征学习 start			
		if( (value == 0x01)){//A5 5A 06 83 00 3C 01 00 01	特征学习 start				
			...
			my_env.workstep =13;	
			Uart0_Printf("start learning %s %d\n", __FILE__, __LINE__);
		}else if( (value == 0x02)){	
		...
		break;
	////////////////A5 5A 06 83 00 22 01 00 0x:1.2/////////////////////////////
	case ADDR_YRUN:  //地址ADDR_YRUN  0X22 按键返回值判断 
		if( (value == 0x01))	{ //预置计数界面 start	
			//A5 5A 06 83 00 22 01 00 01					
			if( (precoin_set[PRECOIN0].pre_count_set >0)||(precoin_set[PRECOIN1].pre_count_set >0)
			||(precoin_set[PRECOIN2].pre_count_set >0)||(precoin_set[PRECOIN3].pre_count_set >0)
			||(precoin_set[PRECOIN4].pre_count_set >0)||(precoin_set[PRECOIN5].pre_count_set >0)
			||(precoin_set[PRECOIN6].pre_count_set >0)||(precoin_set[PRECOIN7].pre_count_set >0) ){	   //只有有预置计数的值了 才可以 转到  预置计数 	
				prepic_num = YZYX;
				comscreen(Disp_Indexpic[YZYX],Number_IndexpicB);  //  to the  picture 
				my_env.workstep = 23;	//预置计数 			
			}
		}else if( (value == 0x02)){
			//A5 5A 06 83 00 22 01 00 02	  预置计数界面 stop						
			STORAGE_MOTOR(STOPRUN);   //斗送入电机
			kickstep = 0;   //踢币检测函数 步骤号
			forgeflag =0;	// 踢币标志位 
			runstep =0; 		
			...
			comscreen(Disp_Indexpic[YZJS],Number_IndexpicB);	 // back to the  picture before alert
			my_env.workstep = 0;		
		}		
		break;
	////////////////////////////////////////////////
	case ADDR_PGH1:  //地址ADDR_PGH1 0X07 
		para_set_value.data.rej_position = (int)touchnum[8];
		dgus_tf1word(ADDR_PGH,para_set_value.data.rej_position);	//make sure  the return one
		Writekick_value();
		my_env.workstep = 0;	// 等待 触摸
		break;
	case ADDR_DBDATA:  //地址ADDR_DBDATA 0X09  按键返回值判断 
		if( (value == 0x01)){	//back value  delete jiemian
		   ...
		   // A5 5A 06 83 00 09 01 00 04  按键值返回 删除当前的总条数 数量 金额 异币	
			Uart0_Printf ("Delete History Data\n");		
			para_set_value.data.total_money = 0;
			para_set_value.data.total_good = 0;
			para_set_value.data.total_ng = 0;
		 ...
		 // A5 5A 06 83 00 09 01 00 06  进入历史数据表格
			my_println ("Enter History Data menu\n");
	...
	case ADDR_TZBC:  //地址ADDR_B1  0X08 按键返回值判断 //back value  / 特征学习 save or not
		if( (value == 0x03)){	//back value 04 混计数界面 xiandan
			...
			if (coinlearnnumber == 0){                                                                         
				my_println ("coin learnnumber = %d, No data to be save", coinlearnnumber); 
			}else{
				prepic_num = TZJM;
				i = is_repeate (my_env.coin_index);
				if (i == 0){
			...
			disp_preselflearn(pre_value.country[coinchoose].coin[my_env.coin_index].data.max0, pre_value.country[coinchoose].coin[my_env.coin_index].data.min0,
							  pre_value.country[coinchoose].coin[my_env.coin_index].data.max1, pre_value.country[coinchoose].coin[my_env.coin_index].data.min1,
							  pre_value.country[coinchoose].coin[my_env.coin_index].data.max2, pre_value.country[coinchoose].coin[my_env.coin_index].data.min2);	
			prepic_num = TZJM;
			...
			my_println ("prepic_num = %d", prepic_num);
			cmd ();
			comscreen(Disp_Indexpic[prepic_num],Number_IndexpicB);	 // back to the  picture before alert
		}
		break;
	case ADDR_PRIT:  //地址ADDR_PRIT 0X39  按键返回值判断 
		if( (value == 0x06)){	//back value 04 混计数界面 xiandan print
			print_func();	 //打印函数
			my_env.workstep = 0;	//停止	所有动作  // 等待 触摸
		}
		break;
//////////////////////////////
	case ADDR_CNTB1:  //地址ADDR_CNTB1 0X3B  图标变量
		my_env.coin_index = (int)touchnum[8];
		my_env.coin_index = my_env.coin_index;
		dgus_tf1word(ADDR_CNTB,my_env.coin_index);	//make sure the tubiao is the return one
		disp_preselflearn(pre_value.country[coinchoose].coin[my_env.coin_index].data.max0, pre_value.country[coinchoose].coin[my_env.coin_index].data.min0,
						  pre_value.country[coinchoose].coin[my_env.coin_index].data.max1, pre_value.country[coinchoose].coin[my_env.coin_index].data.min1,
						  pre_value.country[coinchoose].coin[my_env.coin_index].data.max2, pre_value.country[coinchoose].coin[my_env.coin_index].data.min2);			
		my_env.workstep = 0;	//停止	所有动作  // 等待 触摸
	...
	case ADDR_LEVEL100:  //地址1元硬币的清分等级设置
		...
		Write_qflevel();   //qingfen level write
		my_env.workstep = 0;	//停止	所有动作  // 等待 触摸
		break;
	case ADDR_YZS0:
	case ADDR_YZS1:
...
///////////////////////////////////////////////////////
		...
		break;
	case ADDR_YZ_RESET:  //地址ADDR_YZMZ yuzhi jiemian display clear value
		if( (value == 0x05)){
			int i;
			for (i = 0; i < COIN_TYPE_NUM; i++){
				dgus_tf1word(pre_value.country[COUNTRY_ID].coin[i].data.hmi_state_ico_addr, 0);   //图标  绿
...
//////////////////////////////////////////////////////////////////
	case ADDR_BJZX:  //地址ADDR_BJZX 部件执行
		if( (value == 0x01)){	// bujian zhixing 进入 调试界面						
			detect_read();
...
//			my_env.workstep = 0;	//停止	所有动作  // 等待 触摸
	...
	case ADDR_KICK1_M:  //地址ADDR_KICK1_M bujian zhixing 
		if( (value == 0x00)){	//0灰 1绿   						
			...
			time = para_set_value.data.kick_start_delay_t1; //kick_start_delay_time*1ms	
			Timer3_Start ();	
			dgus_tf1word(ADDR_KICK1_M,1);
			while(time != 0){;}
			EMKICK1(STARTRUN);	  // kick out 
			my_println ("kick1 start");
			time = para_set_value.data.kick_keep_t1;	  //kick_keep_time*1ms
			while(time != 0){;}
			EMKICK1(STOPRUN);	  // kick in 
			my_println ("kick1 stop");
			Timer3_Stop ();	
			dgus_tf1word(ADDR_KICK1_M,0);
		}
		break;
